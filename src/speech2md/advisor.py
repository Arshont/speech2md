...
MODELS = [
    ModelSpec(
        "large-v3", "1.5B", 3.1, "1x (baseline)", "best", "multilingual",
        "https://huggingface.co/Systran/faster-whisper-large-v3",
    ),
    ModelSpec(
        "large-v3-turbo", "0.8B", 1.8, "~5x faster", "near-best", "multilingual",
        "https://huggingface.co/mobiuslabsgmbh/faster-whisper-large-v3-turbo",
    ),
    ModelSpec(
        "distil-large-v3", "0.8B", 1.5, "~6x faster", "near-best", "English ONLY",
        "https://huggingface.co/Systran/faster-distil-whisper-large-v3",
    ),
    ModelSpec(
        "medium", "0.8B", 1.0, "~2x faster", "good", "multilingual",
        "https://huggingface.co/Systran/faster-whisper-medium",
    ),
    ModelSpec(
        "small", "0.2B", 0.5, "~4x faster", "fair", "multilingual",
...
def fits(spec: ModelSpec, hw: HardwareInfo) -> bool:
    if hw.vram_gb is None:
        return hw.ram_gb is None or hw.ram_gb >= spec.vram_gb + 2
    return hw.vram_gb >= spec.vram_gb + VRAM_OVERHEAD_GB
...
def render_table(hw: HardwareInfo) -> str:
    ...
    for spec in MODELS:
        mark = "yes" if fits(spec, hw) else "NO"
        lines.append(
            f"{spec.name:<17} {spec.params:<5} {spec.vram_gb:.1f} GB  {spec.speed:<14} "
            f"{spec.quality:<10} {spec.languages:<13} {mark}"
        )
    lines.append("")
